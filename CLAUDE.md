...
`DATABASE_URL`, `SITE_URL`, `ADMIN_PASSWORD`, `ADMIN_SESSION_SECRET` — см. `.env.local`. Опционально: `RESEND_API_KEY` (без него welcome-письмо просто не отправляется, с warning в лог), `NEXT_PUBLIC_YM_COUNTER_ID` (Яндекс.Метрика).
...
**Публичная часть** (`app/page.tsx`, `components/sections/*`) — одностраничный лендинг, секции собираются в `page.tsx`. Форма регистрации (`components/waitlist-form.tsx`) двухшаговая через `useActionState`:
